...
public class MonthViewDropController extends AbsolutePositionDropController {
	...
	/**
	 * Callback method executed once the drag has completed. We need to reset the background color of all previously highlighted cells. Also need to actually
	 * change the appointment's start / end date here (code doesn't exist yet).
	 * 
	 * @param context
	 *            An object containing information of what was dragged and dropped, including the <code>Appointment</code>, coordinates of the drop, etc.
	 */
	@Override
	...
	/**
	 * Manages the highlighting of cells in the month view grid when an <code>Appointment</code> is being dragged.
	 * 
	 * @param context
	 *            An object providing information about the object being dragged
	 */
	@Override
	...
	/**
	 * Gets all the cells (as DOM Elements) that an appointment spans. Note: It only includes cells in the table. If an appointment ends in the following month
	 * the last cell in the list will be the last cell in the table.
	 * 
	 * @param row
	 *            Appointment's starting row
	 * @param col
	 *            Appointment's starting column
	 * @param days
	 *            Number of days an appointment spans
	...
	protected Element[] getCells(int row, int col, int days) {
		...
		for (int i = 0; i < days; i++) {
			...
			/*
			 * Cheap code here. If the row / cell throw an out of index exception we just break. This kind of sucks because we have to now account for null
			 * items in the Element[] array.
			 */
			try {
