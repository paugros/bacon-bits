	...
	public static void handleRpcExeption(Throwable caught) {
		...
		if (caught instanceof StatusCodeException) {
			...
			if (errorMessage.indexOf(Constants.NOT_AUTHENTICATED_TOKEN) != -1) {
				// if (loginDialog == null) {
				// loginDialog = new LoginDialog((LoginServiceAsync) ServiceCache.getService(LoginService.class), true);
				// }
				// loginDialog.setUsername(Application.getCurrentUser().getUserName());
				// loginDialog.setLoginHandler(new LoginHandler() {
				// @Override
				// public void onLogin(ApplicationData ap) {
				// HistoryToken.setElement("rl", "_", false);
				// }
				// });
				//
				// loginDialog.center();
