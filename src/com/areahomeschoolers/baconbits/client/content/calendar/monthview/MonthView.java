...
import static com.areahomeschoolers.baconbits.client.content.calendar.DateUtils.moveOneDayForward;
import static com.areahomeschoolers.baconbits.client.content.calendar.monthview.MonthViewDateUtils.firstDateShownInAMonthView;
...
import com.areahomeschoolers.baconbits.client.content.calendar.CalendarFormat;
...
import com.areahomeschoolers.baconbits.client.content.calendar.theme.google.client.GoogleMonthViewStyleManager;
...
/**
 * <p>
 * A CalendarView that displays appointments for a given month. The Month is displayed in a grid-style view where cells represents days, columns represents days
 * of the week (i.e. Monday, Tuesday, etc.) and rows represent a full week (Sunday through Saturday).
 * <p/>
 * <p/>
...
public class MonthView extends CalendarView implements HasWeekSelectionHandlers<Date>, HasDaySelectionHandlers<Date> {
	...
	/**
	 * The first date displayed on the MonthView (1st cell.) This date is not necessarily the first date of the month as the month view will sometimes display
	 * days from the adjacent months because of the number of days fitting in the visible grid.
	 */
	...
	/**
	 * The number of rows required to display the entire month in grid format. Although most months span a total of five weeks, there are some months that span
	 * six weeks.
	 */
	private int monthViewRequiredRows = 5;

	/**
	 * List of <code>AppointmentWidget</code>s that are associated to the currently selected <code>Appointment</code> appointment.
	 */
	...
	private MonthViewStyleManager styleManager = GWT.create(GoogleMonthViewStyleManager.class);

	/**
	 * Multiple calculated (&quot;cached&quot;) values reused during laying out the month view elements.
	 */
	...
	/**
	 * This method is called when the MonthView is attached to the Calendar and displayed. This is where all components are configured and added to the
	 * RootPanel.
	 */
	@Override
	public void attach(CalendarWidget widget) {
		...
		/*
		 * Need to re-set appointmentCanvas to position:absolute because gwt-dnd will set it to relative, but then the layout gets f***ed up
		 */
	...
	public void doLayout() {
		...
		MonthLayoutDescription monthLayoutDescription = new MonthLayoutDescription(firstDateDisplayed, monthViewRequiredRows, calendarWidget.getAppointments(),
	...
	/**
	 * Handles the DoubleClick event to determine if an Appointment has been selected. If an appointment has been double clicked the OpenEvent will get fired
	...
	/**
	 * Handles the a single click to determine if an appointment has been selected. If an appointment is clicked it's selected status will be set to true and a
	...
	public void onSingleClick(Element clickedElement, Event event) {
		...
		// calendarWidget.fireDateRequestEvent(cellDate(moreLabels.get(clickedElement)), clickedElement);
		// }
		// }
		// }
	}
	...
	private void buildCalendarGrid() {
		...
		for (int monthGridRowIndex = 1; monthGridRowIndex <= monthViewRequiredRows; monthGridRowIndex++) {
			for (int dayOfWeekIndex = 0; dayOfWeekIndex < DAYS_IN_A_WEEK; dayOfWeekIndex++) {
				...
				configureDayInGrid(monthGridRowIndex, dayOfWeekIndex, date, date.equals(today), date.getMonth() != month, weekNumber);
			}
		}
	}

	/**
	 * Calculates the maximum number of appointments that can be displayed in a given &quot;day cell&quot;.
	 */
	private void calculateCellAppointments() {
		int paddingTop = appointmentPaddingTop();
		int height = appointmentHeight();

		calculatedCellAppointments = (int) Math.floor((calculatedCellHeight - paddingTop) / (height + paddingTop)) - 1;
	}

	/**
	 * Calculates the height of each day cell in the Month grid. It excludes the height of each day's header, as well as the overall header that shows the
	 * weekday labels.
	 */
	...
	/**
	 * Returns the date corresponding to the <code>cell</code> (as if the month view grid was a big linear sequence of cells) in the month view grid.
	 * 
	 * @param cell
	 *            The cell number in the month view grid
	 ...
	 */
	private void configureDayInGrid(int row, int col, Date date, boolean isToday, boolean notInCurrentMonth, int weekNumber) {
	...
	/**
	 * Finds any related <code>AppointmentWidgets</code> associated to the passed Appointment, <code>appt</code>.
	 * 
	 * @param appt
	...
	private void layOnAppointment(Appointment appointment, int colStart, int colEnd, int row, int cellPosition) {
	...
	private void layOnTopOfTheWeekHangingAppointments(WeekLayoutDescription weekDescription, int weekOfMonth) {
		...
		for (int layer = 0; layer < calculatedCellAppointments; layer++) {
			...
			for (AppointmentLayoutDescription weekTopElement : descriptionsInLayer) {
				layOnAppointment(weekTopElement.getAppointment(), weekTopElement.getWeekStartDay(), weekTopElement.getWeekEndDay(), weekOfMonth, layer);
			}
		}
	}
		...
		for (int dayOfWeek = 0; dayOfWeek < DAYS_IN_A_WEEK; dayOfWeek++) {
			...
			if (dayAppointments != null) {
				...
				for (int i = 0; i < count; i++) {
					...
					appointmentLayer = topAppointmentManager.nextLowestLayerIndex(dayOfWeek, appointmentLayer);
				...
				layOnNMoreLabel(topAppointmentManager.multidayAppointmentsOverLimitOn(dayOfWeek), dayOfWeek, weekOfMonth);
			}
		}
	}
	...
	private void placeItemInGrid(Widget panel, int colStart, int colEnd, int row, int cellPosition) {
		...
		float top = calculatedWeekDayHeaderHeight + (row * calculatedCellOffsetHeight) + calculatedDayHeaderHeight + paddingTop
