 ...
 * view. Subclasses provide appropriate appointment styles based on a specific theme using methods
 * {@link #getViewAppointmentStyleForTheme(com.areahomeschoolers.baconbits.client.content.calendar.Appointment)} and
...
public abstract class MonthViewStyleManager {
	...
	/**
	 * Applies a style in the currently selected theme using the runtime-provided theme styles defined by
	 * {@link #getViewAppointmentStyleForTheme(com.areahomeschoolers.baconbits.client.content.calendar.Appointment)} and
	 * {@link #getDefaultViewAppointmentStyleForTheme()}.
	 * 
	 * @param widget
	 *            The widget to style
	 * @param selected
	 *            Indicates if the appointment is the currently selected in the view
	 */
	...
	protected void doApplyStyleInternal(AppointmentWidget widget, boolean selected) {
		...
		Element link = widget.getWidget().getElement();
		// Is MultiDay?
		...
		link.getStyle().setColor(style.getSelectedBorder());
	...
	/**
	 * Returns the default appointment style corresponding to the currently used theme. Subclasses should provide a style to be used if no appropriate style can
	 * be identified ({@link #getViewAppointmentStyleForTheme(com.areahomeschoolers.baconbits.client.content.calendar.Appointment)} returns <code>null</code>).
	 * 
	 * @return The style to use with a theme if no specific style can be identified with
	 *         {@link #getViewAppointmentStyleForTheme(com.areahomeschoolers.baconbits.client.content.calendar.Appointment)}
	 */
	 ...
	 * 
	 * @param appointment
	 *            An appointment to be displayed in the month view, should include a style to be used
	 * @return The style to use with a theme, can be <code>null</code> if a default should be used
	 * @see com.areahomeschoolers.baconbits.client.content.calendar.monthview.MonthViewStyleManager#getDefaultViewAppointmentStyleForTheme()
	 */
