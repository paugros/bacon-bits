...
/**
 * Responsible for arranging all Appointments, visually, on a screen in a manner similar to the Microsoft Outlook / Windows Vista calendar. See: <img
 * src='http://www.microsoft.com/library/media/1033/athome/images/moredone/calendar.gif'/>
 * <p>
...
public class DayViewLayoutStrategy {
	...
	public List<AppointmentAdapter> doLayout(List<Appointment> appointments, int dayIndex, int dayCount) {
		...
		/*
		 * Note: it is important that all appointments are sorted by Start date (asc) and Duration (desc) for this algorithm to work. If that is not the case,
		 * it won't work, at all!! Maybe this is a problem that needs to be addressed
		 */
		 ...
		 * int maxCol = 0;
		 * 
		 * //find the max cell for (AppointmentCell apptCell : block.getAppointments()) { int col = apptCell.getColumnStart(); if (col > maxCol) { maxCol = col;
		 * } }
		 * 
	...
	public int doMultiDayLayout(List<Appointment> appointments, List<AppointmentAdapter> adapters, Date start, int days) {
		...
		for (AppointmentAdapter adapter : adapters) {
			...
			for (int i = 0; i < dateList.size(); i++) {
				...
				// System.out.println("    isWithinRange == " + isWithinRange + " for start: " + adapter.getAppointment().getStart() + " end: " +
				// adapter.getAppointment().getEnd() + " date: "+date);
			...
			for (int x = 0; x < adapters.size(); x++) {
				...
				for (int y = adapter.getColumnStart(); y <= adapter.getColumnStart() + adapter.getColumnSpan(); y++) {
					try {
						...
						// System.out.println("Exception: y=" + y + " x=" + x + " adapters.size=" + adapters.size() +
			...
			// System.out.println("multi-day layout -- title: \"" + adapter.getAppointment().getTitle() + "\" | col start: " + adapter.getColumnStart() +
			// " | colspan: " + adapter.getColumnSpan() + " | row: " + adapter.getCellStart() + " | start date: " + adapter.getAppointment().getStart() +
			// " | end date: " + adapter.getAppointment().getEnd());
