...
public class ServiceCache {
	...
	public static RemoteServiceProxy getService(Class<? extends RemoteService> serviceClass) throws ClientClassNotFoundException {
		...
		if (proxy == null) {
			...
			proxy = (RemoteServiceProxy) Application.getFactory().newInstance(className);

			// // override request builders to centrally register all rpc requests
			...
			// Request r = super.send();
			//
			// Application.registerRpcRequest(r);
			// return r;
			// }
			// };
			// }
			//
			// };
			// ((ServiceDefTarget) proxy).setRpcRequestBuilder(builder);
