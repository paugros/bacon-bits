...
public class DefaultHyperlink extends Widget implements HasHTML, SourcesClickEvents, HasClickHandlers, HasDirectionEstimator, HasDirectionalSafeHtml {
	 ...
	 */
	public DefaultHyperlink(SafeHtml html, Direction dir, String targetHistoryToken) {
	 ...
	 *            A DirectionEstimator object used for automatic direction adjustment. For convenience, {@link #DEFAULT_DIRECTION_ESTIMATOR} can be used.
	 * @param targetHistoryToken
	 *            the history token to which it will link
	 * @see #setTargetHistoryToken
	 */
	public DefaultHyperlink(SafeHtml html, DirectionEstimator directionEstimator, String targetHistoryToken) {
	 ...
	 * @see #setTargetHistoryToken
	 */
	public DefaultHyperlink(SafeHtml html, String targetHistoryToken) {
	 ...
	 * @see #setTargetHistoryToken
	 */
	public DefaultHyperlink(String text, boolean asHTML, String targetHistoryToken) {
		this();
	 ...
	 * @param targetHistoryToken
	 *            the history token to which it will link, which may not be null (use {@link Anchor} instead if you don't need history processing)
	 */
	public DefaultHyperlink(String text, Direction dir, String targetHistoryToken) {
	 ...
	 *            the history token to which it will link, which may not be null (use {@link Anchor} instead if you don't need history processing)
	 */
	public DefaultHyperlink(String text, DirectionEstimator directionEstimator, String targetHistoryToken) {
	 ...
	 *            the history token to which it will link, which may not be null (use {@link Anchor} instead if you don't need history processing)
	 */
	public DefaultHyperlink(String text, String targetHistoryToken) {
	 ...
	 * @see #setTargetHistoryToken
	 */
	private DefaultHyperlink(String text, boolean asHTML, Direction dir, String targetHistoryToken) {
		this();
	 ...
	 * @see #setTargetHistoryToken
	 */
	private DefaultHyperlink(String text, boolean asHTML, DirectionEstimator directionEstimator, String targetHistoryToken) {
		this();
	...
	protected DefaultHyperlink(Element elem) {
		if (elem == null) {
	...
	/**
	 * @deprecated Use {@link Anchor#addClickHandler} instead and call History.newItem from the handler if you need to process the click before the history
	 *             token is set.
	 */
	@Override
	@Deprecated
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Note: DirectionEstimator should be set before the widget has any content; it's highly recommended to set it using a constructor. Reason: if the widget
	 * already has non-empty content, this will update its direction according to the new estimator's result. This may cause flicker, and thus should be
	 * avoided.
	 */
	@Override
	...
	/**
	 * Sets the history token referenced by this hyperlink. This is the history token that will be passed to {@link History#newItem} when this link is clicked.
	 * 
	 * @param targetHistoryToken
	 *            the new history token, which may not be null (use {@link Anchor} instead if you don't need history processing)
	 */
	public void setTargetHistoryToken(String targetHistoryToken) {
		...
		anchorElem.setPropertyString("href", Constants.URL_SEPARATOR + hash);
	}
